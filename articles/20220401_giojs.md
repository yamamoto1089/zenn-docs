...
文章だとイメージが掴みづらいのですが、要するに地球儀の3Dモデルを表示することができます。  
またデータを登録することでデータを地球儀上で可視化できるので、以下画像のような地球儀を表示することができます。
...
**①導入のシンプル性**  
4行のJavaScriptコードで3Dの地球儀モデルを作成することができる  

**②カスタマイズ性**  
Gio.jsのAPIを使用することで地球儀のデザインをカスタマイズすることができる  
...
# 今回試すこと  
Gio.jsを導入し、以下のようなシンプルな地球儀を表示します。  
...
`setStyle`で指定できないような背景色や海の透過率を変更するAPIも用意されているため、APIの組み合わ次第で、好みの色に変更することもできそうです。  
...
# 所感
Gio.jsでは簡単に3Dの地球儀を描画することができ、データを追加することで国同士の関係を可視化できるのが面白いと感じました。  

今回は色を変更した程度でしたが、Gio.jsには他にもAPIが用意されているので、よりインタラクティブな表示にすることもできそうです。
